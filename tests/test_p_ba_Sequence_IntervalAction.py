...
from actions.base import IntervalAction, sequence

# Used to compare floats; a==b <-> abs(a-b)<fe
fe = 1.0e-6

# Recording list for test verification
rec = []


# Mock sprite class to replace CocosNode
class MockSprite(Sprite):
    ...
    def do(self, action):
        """Mimics the CocosNode do() method"""
        action.target = self
        action.start()
        return action

    def _step(self, dt):
        """Mimics the CocosNode _step() method"""
...
class Test_Sequence_IntervalAction:
    ...
    def test_receiving_stop_async_before_duration1(self, duration1, duration2):
        # 0<duration1
        global rec
    ...
    def test_receiving_stop_async_after_duration1_before_duration2(self, duration1, duration2):
        # 0<duration2
        global rec
