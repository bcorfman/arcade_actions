...
def get_closest_sprite(sprite: SpriteType, sprite_list: SpriteList) -> tuple[SpriteType, float] | None:
    """
...
def check_for_collision_with_list(
    ...
    if __debug__:
        if not isinstance(sprite, BasicSprite):
            raise TypeError(
                f"Parameter 1 is not an instance of the Sprite class, " f"it is an instance of {type(sprite)}."
            )
...
def check_for_collision_with_lists(
    ...
    if __debug__:
        if not isinstance(sprite, BasicSprite):
            raise TypeError(
                f"Parameter 1 is not an instance of the BasicSprite class, " f"it is an instance of {type(sprite)}."
            )
