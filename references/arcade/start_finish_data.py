...
class StartFinishRenderData:
    ...
    def __init__(self, pixelated: bool = False, blend: bool = True):
        ...
        self.texture = Texture.create_empty("start_finish_render_texture", size=self.window.get_framebuffer_size())
    ...
    def draw(self):
        ...
        region = LBWH(0, 0, self.texture.width, self.texture.height).scale(min_factor, anchor=AnchorPoint.BOTTOM_LEFT)
        region = region.move(dx=(w - region.width) / 2, dy=(h - region.height) / 2)

        draw_texture_rect(self.texture, region, pixelated=self.pixelated, blend=False, atlas=self.atlas)
