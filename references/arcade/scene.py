...
class Scene:
    ...
    * Flexible but slow support for the ``in`` & ``del`` Python keywords.
    ...
    def add_sprite_list_after(
        ...
        if name in self._name_mapping.keys():
            self.remove_sprite_list_by_name(name)
            warn(f"A Spritelist with the name: '{name}', " f"is already in the scene, will override Spritelist")
        self._name_mapping[name] = sprite_list
    ...
    def update(
        ...
        **kwargs,
        ...
        if not isinstance(delta_time, (int, float)):
            raise TypeError(f"Expected a number for delta_time, but got {type(delta_time)} instead.")

        if names is not None:
            # Due to api changes in 3.0 we sanity names
            if not isinstance(names, Iterable):
                raise TypeError(f"Expected an iterable of layer names, but got {type(names)} instead.")
    ...
    def update_animation(self, delta_time: float, names: Iterable[str] | None = None, *args, **kwargs) -> None:
        """
