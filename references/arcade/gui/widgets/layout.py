...
class UIBoxLayout(UILayout):
    ...
    def do_layout(self):
        ...
        available_space = (self.content_height if self.vertical else self.content_width) - self._space_between * (
            len(self.children) - 1
        )
        ...
        for (child, data), main_size, ortho_size in zip(self._children, main_sizes, orthogonal_sizes, strict=False):
...
class UIGridLayout(UILayout):
    ...
    def _update_size_hints(self):
        ...
        self.size_hint_min = (
            base_width + sum(minimal_width_per_column) + (self.column_count - 1) * self._horizontal_spacing,
            base_height + sum(minimal_height_per_row) + (self.row_count - 1) * self._vertical_spacing,
        )
    ...
    def do_layout(self):
        ...
        column_constraints = [
            _C(minimal_width_per_column[i], None, max_size_hint_per_column[i]) for i in range(self.column_count)
        ]
        ...
        row_constraints = [_C(minimal_height_per_row[i], None, max_size_hint_per_row[i]) for i in range(self.row_count)]
