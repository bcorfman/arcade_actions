...
class Texture:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(self._hit_box_algorithm, HitBoxAlgorithm):
            raise TypeError(
                f"hit_box_algorithm must be an instance of HitBoxAlgorithm, " f"not {type(self._hit_box_algorithm)}"
            )
    ...
    @classmethod
    def create_image_cache_name(cls, path: str | Path, crop: tuple[int, int, int, int] = (0, 0, 0, 0)):
        """
