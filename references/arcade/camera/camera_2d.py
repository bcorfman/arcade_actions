    ...
    def __init__(
        ...
        *,
        ...
        left, right, bottom, top = (
            (-half_width, half_width, -half_height, half_height) if projection is None else projection.lrbt
        )

        if projection is not None:
            if left == right:
                raise ZeroProjectionDimension(f"projection width is 0 due to equal {left=}" f"and {right=} values")
            if bottom == top:
                raise ZeroProjectionDimension(f"projection height is 0 due to equal {bottom=}" f"and {top=}")
        if near == far:
            raise ZeroProjectionDimension(f"projection depth is 0 due to equal {near=}" f"and {far=} values")
