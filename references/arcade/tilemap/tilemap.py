...
class TileMap:
    ...
    def _get_tile_by_id(self, tileset: pytiled_parser.Tileset, tile_id: int) -> pytiled_parser.Tile | None:
    ...
    def _create_sprite_from_tile(
        ...
        if tile.animation:
            ...
            # type: ignore
            cast(TextureAnimationSprite, my_sprite).animation = TextureAnimation(keyframes=key_frame_list)
        ...
        for row_index, row in enumerate(map_array):
            for column_index, item in enumerate(row):
                ...
                if my_sprite is None:
                    print(f"Warning: Could not create sprite number {item} " f"in layer '{layer.name}' {tile.image}")
                else:
                    my_sprite.center_x = (
