<!-- e9738c78-e6a8-403d-995b-652efffc290f 671231f9-a91b-480d-8d73-c62964368e38 -->
# 10× Productivity Roadmap for 1942-Style Shooter with ArcadeActions
...
**Target Game:** Top-down space shooter with large scrolling world maps, formation-based enemy waves, boss encounters, and 8–10 authored levels.

**Expected Outcome:** 3-month tooling investment enables 2-month content creation (vs 6 months all-code), approaching 3× on first game, 10× on subsequent projects.
...
### 1. Hot-Reload Core  (impact ★★★★★, week 1–2)

**Goal:** See code/data changes in-game within ≤1 s, no restart.
...
- For assets: call Scene reload API (§2)
...
- Edit enemy spawn position in Tiled, save → see change in <1 s
- Modify formation behavior Python file → enemies update without losing player progress
...
### 2. Scene & Asset Live-Reload  (★★★★☆, week 2–3)
...
5. Optimize for 2000×10000 px maps: only instantiate sprites in visible viewport + 1-screen buffer

**Exit Criteria**

- 2000×10000 map reloads in <200 ms
...
### 3. Tiled Action Plug-in  (★★★★☆, week 3–4)
...
### 4. Prefab & Inheritance System  (★★★☆☆, week 4–5)
...
### 5. Data-Driven Behaviors & Formations  (★★★☆☆, week 5–6)
...
**Deliverable:** Playable demo with 2 levels, 3 enemy types, 1 boss—all authored in Tiled with hot-reload.
...
### 6. Action Inspector & Time-Travel Debugger  (★★★★☆, week 7–8)
...
- Lists last 20 actions: `[✓] spawn_enemy(bomber, V-shape, 5)` (green) or `[✗] trigger_boss (no target)` (red)
...
### 7. Automated Playtest CLI  (★★★☆☆, week 8–9)
...
### 8. Analytics & Balance Dashboard  (★★★☆☆, week 10)
...
### 9. Parallel Asset Pipeline  (★★☆☆☆, week 11)
...
4. Fingerprint cache: `sha256` of input file → skip if unchanged
5. Integrations:
...
### 10. Opinionated Starter Template  (★★☆☆☆, week 11–12)
...
- Answer 4 prompts
- `cd my_shooter && uv run game.py` → playable demo in <5 min
...
- 8 levels × 3 weeks each = 24 weeks (6 months)

**With Phase 1 tooling:**

- 3-month build + 8 levels × 1 week each = 5 months (17% faster)

**With Phase 1+2 tooling:**

- 3-month build + 8 levels × 0.5 weeks each = 4 months (33% faster)

**Subsequent games (tooling reused):**

- 8 levels × 0.5 weeks = 1 month (6× faster than baseline)
...
- [ ] Implement file-watcher and ActionManager.reload for Python modules & map/asset files
- [ ] Add incremental Scene/TileMap reload preserving entity state
- [ ] Build on-screen action inspector and 3-second rewind buffer
- [ ] Develop Tiled JS plug-in to list & validate actions from schema
- [ ] Introduce YAML prefab/inheritance loader and Tiled templates
- [ ] Load JSON/YAML state machines/BTs from map properties
- [ ] Create headless playtest runner and integrate with CI
- [ ] Embed telemetry hooks and local dashboard
- [ ] Parallelize asset optimization with fingerprint cache
- [ ] Publish cookiecutter template bundling all features